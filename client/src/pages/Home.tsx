...
export function Home() {
  ...
  const trendingTopics = ['AI', 'Machine Learning', 'Web Development', 'Space Exploration', 'Climate Change', 'Laos Stock-Market'];
  const handleClick = (topic: string) => {
    setQuery(topic); // Update the query state when a topic is clicked
    ...
    <div className="min-h-screen flex flex-col items-center justify-center bg-background">
      ...
      <div className="w-full max-w-3xl px-4 animate-fade-in">
          ...
          <div className="flex flex-wrap gap-4 mt-4">
            {trendingTopics.map((topic) => (
              <Button
                ...
                className="px-4 py-2 text-sm font-medium text-white bg-blue-500 hover:bg-blue-600 dark:bg-gray-700 dark:hover:bg-gray-600 rounded-full transition-colors"
              >
                {topic}
              </Button>
            ))}
        ...
        <div className="mt-8 text-center text-sm text-gray-500 dark:text-gray-400 animate-fade-in space-y-1">
          ...
          <div>
            Created by <a href="https://www.linkedin.com/in/visitxay-hanmonty/" target="_blank" rel="noopener noreferrer" className="hover:text-gray-800 dark:hover:text-gray-300 transition-colors">@XayHanmonty</a>
          </div>
        </div>
      </div>
    </div>
